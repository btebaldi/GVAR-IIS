...
DX.df <- tbl %>%
  select("brent", "lnCambio", colunas_Selecionadas) %>%
...
X.df <- tbl %>% select("brent", "lnCambio", colunas_Selecionadas)
...
colnames(Forecast) <- c("F_Brent", "F_lnCambio",
                        paste("R",
                              sort(rep(1:110,3)),
...
for(i  in seq_along(regiao)){
  ...
  ggsave(filename = sprintf("./Graficos/%s/Forecast GVAR-IIS - Diesel - %s.png", dir, names(regiao)[i]),
         plot = g1,
         units = "in",
         width = 8, height = 6,
         dpi = 100)
  ...
  gcow <- cowplot::plot_grid(g11, g12, g13,
                           nrow=1, ncol = 3)
  
  
  ggsave(filename = sprintf("./Graficos/%s/Forecast GVAR-IIS - All 2019 plus -%s.png", dir, names(regiao)[i]),
         plot = gcow,
         units = "in",
         width = 8*3, height = 6,
         dpi = 100)
