...
for (modelo_atual in lista_de_modelos) {
  ...
  file.name <- sprintf("Erro_Modelo_%d.rds", modelo_atual)
  dir <- sprintf("Result %d", modelo_atual)
  
  
  export_file <- file.path("Ox", "mat_files", "Result_Matrix", dir, file.name)
  main_path <- dirname(export_file)
  
  print(sprintf("Analisando modelo %d", modelo_atual))
  ...
  colnames(tbl.erro) <- c("DATA_INICIAL", "Serie", sprintf("Erro_GVARIIS_M%d", modelo_atual), 
    sprintf("SE_GVARIIS_M%d", modelo_atual))
