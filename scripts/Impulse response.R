...
for(regiao in c(75, 62,109,46,39,20,4)){
  g1 <- response.df %>% 
    mutate_all(.funs = mDiff) %>%
    select(Etanol = sprintf("R_%d_E",regiao), 
           Diesel = sprintf("R_%d_D",regiao), 
           Gasolina = sprintf("R_%d_G",regiao))  %>% 
    mutate(Id = row_number()) %>% 
    ggplot() +
    geom_line(aes(x=Id, y=Etanol, colour="E")) +
    geom_line(aes(x=Id, y=Diesel, colour="D")) +
    geom_line(aes(x=Id, y=Gasolina, colour="G")) +
    geom_hline(yintercept = 0) +
    labs(title = sprintf("%d", regiao))
